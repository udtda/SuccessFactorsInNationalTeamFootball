...
brant_cr=function(index, print.res){
  ...
  Xtest=read.csv("Data_clean_2016.csv", header=TRUE, row.names = 1)
  Xtest=Xtest[,index]
  setwd('..')
  setwd('..')
  y=factor(read.csv("WebScraping/2016/Data_complete_2016.csv", header=TRUE, row.names = 1)$Result.W.L.D, ordered = TRUE)
  setwd('./Analyse/2016')
  J=3
  K=dim(Xtest)[2]
