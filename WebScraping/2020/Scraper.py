...
def scraper(url, first):
    ...
    soup4 = BeautifulSoup(driver.page_source, features="html.parser")
    driver.quit()
    distanceBox=soup4.find("div", class_="match-statistics--distance-run stats-visualization--horizontal-bar match-statistics--item")
    homeD=distanceBox.find("div", class_="distance-run--value graph-bar--number-value graph-bar--number-value__home-team")
    distanceHome=float(homeD.text.strip())
    awayD=distanceBox.find("div", class_="distance-run--value graph-bar--number-value graph-bar--number-value__away-team")
