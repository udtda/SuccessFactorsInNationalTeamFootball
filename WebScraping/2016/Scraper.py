...
def scraper(url, first):
    ...
    #Scraping of Tackle rate and passing rate from kicker.de
    ...
    possRow=rows[6]
    possHomeS=possRow.find("div", class_="kick__stats-bar__value kick__stats-bar__value--opponent1").text.strip()
    possHome=int(possHomeS[0:(len(possHomeS)-1)])
    possAwayS=possRow.find("div", class_="kick__stats-bar__value kick__stats-bar__value--opponent2").text.strip()
    possAway=int(possAwayS[0:(len(possAwayS)-1)])
    tackleRow=rows[7]
    ...
    if first:
        header.append("Ball possession")
    home.append(possHome)
    away.append(possAway)
    if first:
    ...
    if first:
        file=open("Data_complete_2016.csv", "w", newline="")
    ...
    if first is False:
        file=open("Data_complete_2016.csv", "a", newline="")
        writer=csv.writer(file)
        writer.writerow(home)
        writer.writerow(away)
...
def addColEnd(col):
    i=0
    with open("Data_complete_2016.csv", "r", newline="") as input:
        with open("Data_complete_2016_addOn.csv", "w", newline="") as output:
